...
static int post_process_retinaface(rknn_app_context_t *app_ctx, image_buffer_t *src_img, rknn_output outputs[], retinaface_result *result, letterbox_t *letter_box) {
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        for (int j = 0; j < 5; ++j) { // Facial feature points
            float point_x = landms[n * 10 + 2 * j] * app_ctx->model_width - letter_box->x_pad;
            float point_y = landms[n * 10 + 2 * j + 1] * app_ctx->model_height - letter_box->y_pad;
            result->object[last_count].point[j].x = (int)(clamp(point_x, 0, model_in_w) / letter_box->scale);
            result->object[last_count].point[j].y = (int)(clamp(point_y, 0, model_in_h) / letter_box->scale);
        }
        last_count++;
    }
