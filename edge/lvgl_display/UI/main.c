...
//     if (behav_res) {
//         int score = 0;
//         if (behav_res->phone_call) score += 20;
//         if (behav_res->smoking)    score += 20;
//         if (behav_res->head_down)  score += 30;
//         // if (behav_res->eye_closed) score += 30;
//         if (score > 100) score = 100;
//         ui_update_fatigue_score(score);

//         ui_update_perclos(score / 2);
//         ui_update_blink_freq(12 + score / 10);
//         ui_update_yawn_count(score / 30);
//         ui_update_eye_state(behav_res->head_down ? "Tired" : "Normal");
//     }

//     // 风扇状态与芯片温度
//     if (fan_ctrl) {
//         if (fan_ctrl->mode == 0)      ui_update_fan_mode("Auto");
//         else if (fan_ctrl->mode == 2) ui_update_fan_mode("Manual");
//         else if (fan_ctrl->mode == 3) ui_update_fan_mode("Off");
//         else                          ui_update_fan_mode("Unknown");

//         // 仅在自动模式下更新滑块显示，手动模式保持用户当前值
//     if (fan_ctrl->mode == 0) {
//         ui_update_fan_speed(fan_ctrl->current_speed);
//     }else if (fan_ctrl->mode == 2) {
//         ui_update_fan_speed(fan_ctrl->manual_speed);   // 直接显示用户设定的值
//     }
//     ui_update_chip_temp(fan_ctrl->chip_temp);


//         // 显示实时风速（由 fan_control 计算）
//         ui_update_fan_speed(fan_ctrl->current_speed);
//         // 显示芯片温度
//         ui_update_chip_temp(fan_ctrl->chip_temp);
//     }
...
//         if (env_data != (void*)-1) {
//             printf("[UI] EnvData connected\n");
//         } else {
//             printf("[UI] EnvData attach failed\n");
//         }
//     } else {
//         printf("[UI] EnvData shmget failed, key=0x%x\n", ENV_SHM_KEY);
//     }
...
//         if (behav_res == (void*)-1) behav_res = NULL;
//     }
...
//         if (fan_ctrl == (void*)-1) fan_ctrl = NULL;
//     }

//     // 将风扇控制指针传递给 UI 模块（用于滑块等回调）
//     ui_set_fan_ctrl(fan_ctrl);

//     // ----- LVGL 初始化 -----
//     lv_init();
//     fbdev_init();          // framebuffer 驱动
//     evdev_init();          // 触摸驱动（需已配置 EVDEV_NAME）
...
//         ui_update_fatigue_score(out.fatigue_score);
//         ui_update_eye_state(out.eye_text);
//         ui_update_perclos(out.perclos);
//         ui_update_blink_freq(out.blink_freq);
//         ui_update_yawn_count(out.yawn_count);
//     } else {
//         ui_update_fatigue_score(0);
//         ui_update_eye_state("No Data");
//         ui_update_perclos(0);
//         ui_update_blink_freq(0);
//         ui_update_yawn_count(0);
//     }
...
// uint32_t custom_tick_get(void) { return (uint32_t)get_now_ms(); }
